...
const
  OpenTagKind* = ParLe    ## build-agnostic `case n.kind` label for a tag head
  StrLitKind* = StringLit  ## build-agnostic `case n.kind` label for a string lit
  UnknownTokenKind* = UnknownToken
  EofTokenKind* = EofToken

proc isTagLit*(c: Cursor): bool {.inline.} = c.kind == ParLe
proc isTagLit*(n: PackedToken): bool {.inline.} = n.kind == ParLe
proc isDotToken*(n: PackedToken): bool {.inline.} = n.kind == DotToken
proc isUnknownToken*(n: PackedToken): bool {.inline.} = n.kind == UnknownToken
proc isUnknownToken*(c: Cursor): bool {.inline.} = c.kind == UnknownToken
proc isIdent*(n: PackedToken): bool {.inline.} = n.kind == Ident
proc isSymbol*(n: PackedToken): bool {.inline.} = n.kind == Symbol
proc isSymbolDef*(n: PackedToken): bool {.inline.} = n.kind == SymbolDef
proc isStringLit*(n: PackedToken): bool {.inline.} = n.kind == StringLit
proc isIntLit*(n: PackedToken): bool {.inline.} = n.kind == IntLit
proc isUIntLit*(n: PackedToken): bool {.inline.} = n.kind == UIntLit
proc isFloatLit*(n: PackedToken): bool {.inline.} = n.kind == FloatLit
proc isCharLit*(n: PackedToken): bool {.inline.} = n.kind == CharLit
proc isDotToken*(c: Cursor): bool {.inline.} = c.kind == DotToken
proc isIdent*(c: Cursor): bool {.inline.} = c.kind == Ident
proc isSymbol*(c: Cursor): bool {.inline.} = c.kind == Symbol
proc isSymbolDef*(c: Cursor): bool {.inline.} = c.kind == SymbolDef
proc isStringLit*(c: Cursor): bool {.inline.} = c.kind == StringLit
proc isIntLit*(c: Cursor): bool {.inline.} = c.kind == IntLit
proc isUIntLit*(c: Cursor): bool {.inline.} = c.kind == UIntLit
proc isFloatLit*(c: Cursor): bool {.inline.} = c.kind == FloatLit
proc isCharLit*(c: Cursor): bool {.inline.} = c.kind == CharLit
  ## Build-agnostic token predicates (mirrored in nifcore_compat) so the sem
  ## port's `n.isTagLit` / `n.isDotToken` / … compile in the classic build too.
