...
## Create an index file for a NIF file. nifprims-based variant.

import std / [tables, assertions, hashes, syncio, strutils]
import bitabs, lineinfos, nifreader, nifprims, nifchecksums2, symparser, vfs
...
proc processForChecksum(dest: var Sha1State; content: var TokenBuf) =
  ...
  # Outer is `(index ...)`. Walk it using the new into/hasMore idiom; each
  # entry is a sealed tag whose subtree we can either skim for checksum
  # purposes or skip wholesale.
  n.into:
    while n.hasMore:
