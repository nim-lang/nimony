...
proc skipIntentMatches*(k: NivKind; intent: SkipIntent): bool {.inline.} =
...
type
  TagClass* = enum
    Anything    ## any single token (any NivKind except EofToken)
...
proc tagClassMatches*(k: NivKind; expected: TagClass): bool {.inline.} =
  case expected
