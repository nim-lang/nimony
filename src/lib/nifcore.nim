...
proc forgeCrucialFile*(origins: openArray[CrucialOrigin];
                       realFile: string): string {.noSideEffect.} =
  ## Build the forged filename for code expanded from `origins` (outermost
  ## first) that physically lives in `realFile`.
  result = CrucialPrefix
  for o in origins:
    result.add o.sym
    result.add CrucialFieldSep
    result.add o.declFile
    result.add CrucialFieldSep
    result.add $o.declLine
    result.add '\0'
  result.add realFile
