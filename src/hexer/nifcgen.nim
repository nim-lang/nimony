...
proc traverseField(c: var EContext; n: var Cursor; flags: set[TypeFlag] = {}) =
  ...
  if prag.externName.len > 0:
    c.registerMangle(s, prag.externName & ".n")
  ...
  of ParLe:
    ...
    of IntT, UIntT:
      ...
      if n.kind != ParRi and n.pragmaKind in {ImportcP, ImportcppP}:
        c.dest.shrink start
        inc n
        c.dest.addSymUse pool.syms.getOrIncl(pool.strings[n.litId] & ".n"), n.info
        inc n
        skipParRi c, n
        skipParRi c, n
      else:
...
proc traverseProc(c: var EContext; n: var Cursor; mode: TraverseMode) =
  ...
  if prag.externName.len > 0:
    c.registerMangleInParent(s, prag.externName & ".n")
...
proc traverseStringCase(c: var EContext; n: var Cursor): bool =
  var n = n
  inc n
  let selectorType = getType(c.typeCache, n)
...
proc writeOutput(c: var EContext, rootInfo: PackedLineInfo) =
  var b = nifbuilder.open(c.dir / c.main & ".n.nif")
...
proc expand*(infile: string, bits: int) =
  ...
  var c0 = setupProgram(infile, infile.changeFileExt ".n.nif", true)
