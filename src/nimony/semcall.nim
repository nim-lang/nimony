...
proc semTemplateCall(c: var SemContext; dest: var TokenBuf; it: var Item; fnId: SymId; beforeCall: int;
  ...
  if res.status == LacksNothing:
    ...
    # line-info filename of the emitted tokens - `__crucial\0<fn>\0<realfile>` -
    ...
    # already expanded and forged its own name by now, so this pass prepends the
    # outer level onto the existing chain and the order comes out outermost
    # first, which is what nesting `inlinedAt` needs.
    #
