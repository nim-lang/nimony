...
proc semBoolExpr(c: var SemContext; n: var Cursor) =
  ...
  if classifyType(c, t) != BoolT:
    ...
    if c.dest.len > start:
      needsParRi = true
      # combine the node generated by semExpr and ErrT node and put them in one StmtsS node.
      if c.dest[start].stmtKind == StmtsS:
        assert c.dest[c.dest.len - 1].kind == ParRi
        c.dest.shrink(c.dest.len - 1)
      else:
        c.dest.insert [parLeToken(StmtsS, c.dest[start].info)], start
