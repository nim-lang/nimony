...
## nifcore port of `induction_variables.nim`. Replaces `(at arr iv)` accesses
## inside a loop with `(deref p)` for a pointer `p` initialized before the loop
## and advanced one element per iteration. See the original module for the full
## pattern/rewrite description; this version runs over **nifcore** cursors and
## uses [patchsets_nc](patchsets_nc.nim) for the structural mutations.
...
when isMainModule:
  ...
  echo "induction_variables_nc.nim: all self-tests passed"
