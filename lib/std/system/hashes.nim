# Hashing for the types `system` declares. Lives in `system` so that every
# operation is attached to its type (`hash(string)`, `hash(int64)`, ... are found
# wherever the type is) and `Hashable`/`Keyable` checks see them without an
# import. `std/hashes` remains as an empty module for `import std/hashes`.
