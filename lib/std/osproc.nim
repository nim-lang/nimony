...
proc running*(p: Process): bool {.raises: [OSError], tags: [].}
...
proc waitForExit*(p: Process, timeout: int = -1): int {.raises: [OSError, ValueError], tags: [TimeEffect].}
...
proc peekExitCode*(p: Process): int {.raises: [OSError], tags: [].}
  ## Return `-1` if the process is still running. Otherwise the process' exit code.
  ##
  ## On posix, if the process has exited because of a signal, 128 + signal
  ## number will be returned.

proc inputStream*(p: Process): Stream {.tags: [].}
...
proc outputStream*(p: Process): Stream {.raises: [IOError, OSError], tags: [].}
...
proc errorStream*(p: Process): Stream {.tags: [].}
...
proc peekableOutputStream*(p: Process): Stream {.tags: [], since: (1, 3).}
  ## Returns ``p``'s output stream for reading from.
  ##
...
proc peekableErrorStream*(p: Process): Stream {.tags: [], since: (1, 3).}
  ## Returns ``p``'s error stream for reading from.
  ##
...
proc inputHandle*(p: Process): FileHandle {.raises: [], tags: [].} =
...
proc outputHandle*(p: Process): FileHandle {.raises: [], tags: [].} =
...
proc errorHandle*(p: Process): FileHandle {.raises: [], tags: [].} =
...
proc countProcessors*(): int {.raises: [].} =
...
proc execProcesses*(cmds: openArray[string],
  ...
  int {.raises: [ValueError, OSError, IOError],
