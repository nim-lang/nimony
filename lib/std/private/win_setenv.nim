...
when not defined(windows): discard
  ...
  var genviron {.importc: "_environ", nodecl.}: ptr ptr char
  ...
  proc setEnvImpl*(name: string, value: string, overwrite: cint): cint =
    ...
    var name = name
    let wideName: WideCString = newWideCString(name).toWideCString()
    if overwrite == 0 and c_wgetenv(cast[ptr wchar_t](wideName)) != nil:
      return 0

    if value != "":
      var newName = name & "=" & value
      let envstring: WideCString = newWideCString(newName).toWideCString
    ...
    if setEnvironmentVariableW(wideName, value.newWideCString().toWideCString()) == 0:
      errno = EINVAL
      return -1
    return 0
