...
| `(interrupt STR)` | NimonyPragma, LengPragma | the `{.interrupt: "SysTick".}` **proc pragma**: this routine is the handler for the named exception or interrupt vector, and the back end installs its address in the vector table. WHICH names exist is a target question — arkham's, exactly as for `(register)` — so sem checks only what is true of every part: a handler is invoked by hardware, which passes no arguments and has nowhere to put a result, so the routine must take no parameters and return nothing. It is also a DCE ROOT: nothing in the program calls it, it is reached only through the table, and without that it is deleted and the device silently never responds. The C and LLVM backends refuse it — they have no vector table to install anything into, and `{.exportc.}` is what binds a handler by name against a vendor startup file |
