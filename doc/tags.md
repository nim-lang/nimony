...
| `(proctype .Any (params...) T P)` | NimonyType, NiflerKind | Nimony proc type. Slot 0 carries the nilability tag — either a `.` placeholder or one of `(notnil)`, `(nil)`, `(unchecked)`. The legacy proc-shaped name/export/pattern/typevars and trailing effects/body slots are gone. |
| `(proctype ...)` | NifcType | NIFC proc type, same shape as `(proc D ...)` with anonymous name slot (varargs spec; effects/body slots present but unused). |
