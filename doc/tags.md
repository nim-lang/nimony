...
| `(uncheckedCycle)` | NimonyPragma | `uncheckedCycle` marker; only valid inside `{.cast(uncheckedCycle).}:` pragma blocks; suppresses the static cycle-prevention check (see doc/nocycles.md) inside the block |
